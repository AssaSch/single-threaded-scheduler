...
typedef enum Vector_Result {
	...
	VECTOR_INDEX_OUT_OF_BOUNDS_ERROR,       /**< index smaller then 1 or larger then number of elements on set/get functions  */ 
	VECTOR_OVERFLOW_ERROR,                  /**< vector is full and block size is 0 on append   */ 
	VECTOR_UNDERFLOW_ERROR                  /**< vector is empty on remove   */
 ...
 * @warning can append null pointer.
 */
 ...
 * @param[out] _pItem - pointer to variable that will receive deleted item value
 * @return success or error code 
 * @retval VECTOR_SUCCESS on success 
 * @retval VECTOR_UNITIALIZED_ERROR 
 ...
 * @param[out] _pItem - pointer to variable that will recieve the item's value.
 * @return success or error code 
 * @retval VECTOR_SUCCESS on success 
 * @retval VECTOR_UNITIALIZED_ERROR 
